...
- 2026-02-13: Gender added as optional intake field with Male/Female options (Male default in UI).
- 2026-02-13: Imperial height uses preset dropdown from 4ft 0in to 7ft 0in (default 5ft 9in).
...
## Commit Conventions
- `src/tailwind.css` is autogenerated by Tailwind build/test scripts.
- Do not block on large generated diffs in `src/tailwind.css`.
- Include `src/tailwind.css` in a separate commit from functional code changes.
